...
<template>
  <Panel
    no-hover
    class="group relative flex min-h-[148px] cursor-pointer flex-col border border-line bg-surface p-1.5 text-center shadow-[0_0_0_1px_rgba(255,255,255,0.02)] transition-all duration-300 hover:border-blue-600 max-[390px]:min-h-[142px] max-[360px]:min-h-[138px] sm:min-h-[188px] sm:p-3 lg:min-h-[214px]"
    @click="goToMatch"
  >
    <div class="mb-0.5 flex min-h-[18px] items-center justify-between gap-1 sm:mb-1.5 sm:min-h-[22px] sm:gap-2">
      <button
        ...
        class="flex min-w-0 items-center gap-1 text-[11px] text-muted transition-colors hover:text-blue-500 sm:gap-1.5 sm:text-[12px]"
      ...
      <div v-else class="flex min-w-0 items-center gap-1 text-[11px] text-muted sm:gap-1.5 sm:text-[12px]">
        <LogoFrame v-if="match.leagueLogo" :src="match.leagueLogo" size="cardLeague" />
        <span class="truncate">{{ match.leagueName }}</span>
      </div>
    ...
    <div class="flex flex-1 flex-col justify-center px-0 py-0.5 sm:px-1 sm:py-1">
      <div class="mb-0.5 grid w-full grid-cols-[1fr_auto_1fr] items-center sm:mb-1.5">
        ...
        >
          ...
          <span class="inline-flex h-6 w-6 items-center justify-center rounded-full border border-line-strong bg-surface text-[8px] text-subtle sm:h-8 sm:w-8 sm:text-[9px] lg:h-10 lg:w-10" v-else>Gazde</span>
        </button>
        ...
        >
          ...
          <span class="inline-flex h-6 w-6 items-center justify-center rounded-full border border-line-strong bg-surface text-[8px] text-subtle sm:h-8 sm:w-8 sm:text-[9px] lg:h-10 lg:w-10" v-else>Oaspeti</span>
        </button>
      </div>
      ...
      <div class="flex flex-col items-center">
        <div class="grid w-full grid-cols-[1fr_auto_1fr] items-center gap-1 max-[390px]:gap-0.5 sm:gap-1.5">
          <button
            class="min-w-[24px] justify-self-end text-right text-[11px] font-black tracking-normal text-fg transition-colors group-hover:text-blue-500 hover:underline sm:min-w-[38px] sm:text-[15px]"
            @click.stop="goToTeam(match.homeTeamId)"
          >
            {{ getTeamTag(match.homeTeamName, match.homeTeamTla) }}
          </button>
          <span class="px-0.5 text-center text-[8px] font-semibold tracking-[0.05em] text-subtle max-[390px]:text-[7px] sm:text-[10px] sm:tracking-[0.08em]">vs</span>
          <button
            class="min-w-[24px] justify-self-start text-left text-[11px] font-black tracking-normal text-fg transition-colors group-hover:text-blue-500 hover:underline sm:min-w-[38px] sm:text-[15px]"
            @click.stop="goToTeam(match.awayTeamId)"
          >
        ...
        <p class="mt-0.5 text-[8px] text-muted max-[390px]:text-[7px] sm:mt-1 sm:text-[10px]">{{ formatDateShort(match.startTime) }}</p>
      </div>
    </div>

    <div v-if="match.ftHome !== null && match.ftAway !== null" class="mb-2 text-center">
      <span class="rounded border border-line bg-surface-2 px-1.5 py-0.5 font-mono text-[9px] font-bold text-fg max-[390px]:px-1 max-[390px]:text-[8px] sm:px-3 sm:text-base">
